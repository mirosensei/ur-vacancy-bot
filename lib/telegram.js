...
const NBSP = " ";          // Unicode 非断空格，HTML 下不被合并
...
class Telegram {
  ...
  static propNameHtml(prop) {
    ...
    return prop.url ? `<a href="${Telegram.esc(prop.url)}">${name}</a>` : name;
  }
  ...
  static fmtRoomHtml(r, roomUrl) {
    ...
    // 租金
    const rentRaw = r.rent || r.rent_normal || "—";
    const cfRaw = r.commonfee;
    const hasCf = cfRaw && cfRaw !== "0円";

    // リノベーション标签
    const design = r.design;
    const hasReno = Array.isArray(design) && design.some(d => d["デザイン名"] === "リノベーション住宅");
    ...
    // 第二行缩进与第一行 room name 对齐（dash 悬挂），用 Unicode 非断空格防 HTML 合并
    const parts = [];
    if (hasCf) parts.push(`管 ${Telegram.esc(cfRaw)}`);
    parts.push(type, fs, floor);
    if (hasReno) parts.push("リ");
    const detail = parts.join(" · ");

    return `- ${label} · 租 ${Telegram.esc(rentRaw)}\n${NBSP}${NBSP}${NBSP}${NBSP}${detail}`;
  }
